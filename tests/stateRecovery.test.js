 ...
 *  5. Reconnection state recovery       – restoreUserStateAfterReconnect and
 *                                         cleanupBusyUsersOnStartup via stateService
 *  6. Race conditions on reconnect      – simultaneous reconnects allow only one login
 */
...
// ── Shared test server ────────────────────────────────────────────────────────
// A single server instance is used for all socket-state tests to honour the
// module-level state inside socketController (onlineUsers, roomMembers, …).
// Each test uses unique nick names to guarantee isolation.
...
// ── Per-test harness (used by reconnect race-condition tests) ─────────────────
...
/**
 * Wait for either 'login-success' or 'login-error' on a socket.
 * Resolves with { eventName, payload } so callers can distinguish outcomes.
 */
const waitForLoginOutcome = (socket) => {
  return Promise.race([
    waitForEvent(socket, 'login-success').then((payload) => ({ eventName: 'login-success', payload })),
    waitForEvent(socket, 'login-error').then((payload) => ({ eventName: 'login-error', payload }))
  ]);
};
...
test('simultaneous reconnects for the same user allow only one login', async (t) => {
  ...
  const clientOne = ioClient(harness.url, {
    auth: { sid: 'shared-session-id' },
    transports: ['websocket']
  });
  const clientTwo = ioClient(harness.url, {
    auth: { sid: 'shared-session-id' },
    transports: ['websocket']
  });
